...
export function FileMessage({ file }: { file: FileAttachment }) {
  ...
  if (file.isImage) {
    return (
      <div className="max-w-xs overflow-hidden rounded-lg border border-line dark:border-gray-700">
        <div
          data-thumb
          className="flex h-40 items-center justify-center bg-gradient-to-br from-brand/40 to-surface-2 text-muted"
        >
  ...
  return (
    <button
      ...
      className="flex items-center gap-3 rounded-lg border border-line bg-white px-3 py-2 text-left hover:border-brand dark:border-gray-700 dark:bg-gray-800"
    >
      ...
      <span className="min-w-0">
        <span className="block truncate text-sm text-ink dark:text-white">{file.name}</span>
