            ...
            <div className="tl-stagger flex gap-3 overflow-x-auto pb-1">
              {recent.map((r) => (
                <button
                  ...
                  className="flex w-20 shrink-0 flex-col items-center gap-1.5 rounded-lg p-2 text-center transition-transform duration-150 ease-[var(--ease-out)] hover:bg-surface-2 active:scale-95"
                >
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-brand-50 text-sm font-semibold text-brand">
                    {initials(r.name)}
                  </span>
                  <span className="w-full truncate text-xs font-medium text-ink-2">
                    {r.name}
                  </span>
                </button>
              ))}
