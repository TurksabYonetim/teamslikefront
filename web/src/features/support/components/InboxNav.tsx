...
const chip = (active: boolean) =>
  clsx("rounded-md border px-2 py-0.5 text-sm font-medium transition-colors duration-[var(--dur-press)] ease-[var(--ease-out)] motion-safe:active:scale-[0.97] motion-safe:transition-[transform,background-color,color]", active ? "border-brand text-brand" : "border-line text-muted hover:bg-surface-2");
...
export function InboxNav() {
  ...
  const inboxBtn = (active: boolean) =>
    clsx("flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-colors duration-[var(--dur-press)] ease-[var(--ease-out)] motion-safe:active:scale-[0.98] motion-safe:transition-[transform,background-color,color]", active ? "bg-surface-2 text-ink" : "text-muted hover:bg-surface-2");
  ...
  return (
    <div className="flex flex-col gap-3 overflow-y-auto rounded-xl border border-line bg-surface p-3">
      ...
      <div>
        <h3 className="mb-1 text-sm font-semibold text-muted">{t("filterHeader")}</h3>
      ...
      <div>
        <h3 className="mb-1 text-sm font-semibold text-muted">{t("assigneeHeader")}</h3>
        <div className="flex flex-wrap gap-1">
          {ASSIGNEES.map((a) => (
